...
# 复制docker相关文件到bin目录
cp -r ../docker/Dockerfile ${project_root_dir}bin/Dockerfile
cp -r ${toml_path}config.toml ${project_root_dir}bin/
...
# （可选）运行Docker镜像
docker run -p 8080:8080 --name ${app_name} -d --restart=on-failure --log-opt max-size=5m -m 1000m ${app_name}:${app_version} /app/main -conf=/app/config.toml -key="$decryption_key"